...
Marker file `~/.souschef/onboarding_complete` gates the flow. Existing users auto-skip.
...
- **Database:** SQLite locally (`~/.souschef/souschef.db`), PostgreSQL in production. `DATABASE_URL` env var for connection.
...
- **DB moved to `~/.souschef/`** — prevents cloud sync corruption of SQLite
