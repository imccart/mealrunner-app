...
def _run_column_migrations(conn: DictConnection) -> None:
    ...
    migrations = [
        ...
        ("trip_items", "removed_at", "TEXT"),
        ("trip_items", "buy_elsewhere", "INTEGER NOT NULL DEFAULT 0"),
...
def _migrate_onboarding_marker(conn: DictConnection) -> None:
    ...
    marker = Path.home() / ".mealrunner" / "onboarding_complete"
    if marker.exists():
...
def _migrate_create_default_user(conn: DictConnection) -> None:
    ...
    conn.execute(text(
        """INSERT INTO users (id, email, display_name, created_at)
           VALUES ('default', 'owner@mealrunner.app', '', CURRENT_TIMESTAMP)"""
    ))

    # Add to whitelist
    conn.execute(text(
        """INSERT INTO allowed_emails (email)
           VALUES ('owner@mealrunner.app')
           ON CONFLICT DO NOTHING"""
    ))
...
def _migrate_stores_to_db(conn: DictConnection) -> None:
    ...
    import json
    stores_file = Path.home() / ".mealrunner" / "stores.json"
    ...
    try:
        ...
        try:
            ...
            if os.environ.get("ENCRYPTION_KEY"):
                from mealrunner.kroger import _encrypt_token, _decrypt_token
