...
def parse_receipt_image(image_path: str, grocery_names: list[str] | None = None) -> tuple[list[dict], int | None]:
    ...
    if grocery_names:
        matching_rules = (
            ...
            "- EXPAND RECEIPT ABBREVIATIONS before matching. Receipts heavily "
            "abbreviate words and prefix items with department codes:\n"
            "    Department prefixes (drop them): DLI/DELI, BKY/BAKERY, "
            "GRC/GROCERY, MEAT, PROD/PRODUCE, FRZ/FROZEN, DRY, DAIRY, FLR/FLORAL\n"
            "    Word abbreviations: MOZZ=mozzarella, CHKN=chicken, CHIX=chicken, "
            "BNANA/BAN=banana, VEG=vegetable, LB=pound, LGS=logs/sticks, "
            "SLCD=sliced, SHRD=shredded, ORG=organic, GF=gluten-free, "
            "WHL=whole, RDC=reduced, FF=fat-free, LF=low-fat\n"
            "    Example: 'DLI MOZZ LGS' = deli mozzarella logs/sticks "
            "→ matches 'mozzarella cheese' or 'mozzarella'\n"
            "    Example: 'GRC CHKN BRST' = grocery chicken breast "
            "→ matches 'chicken breast' or 'chicken'\n"
            "- Match items in any category — food, personal care, household, "
            "cleaning, pet supplies.\n"
            "- Confidence calibration:\n"
            "    Match confidently when an abbreviation has a clear, common "
            "expansion that aligns with a list item.\n"
            "    Set null only when the abbreviation is genuinely ambiguous "
            "(could plausibly expand to multiple unrelated things).\n"
            "    Hallucinated matches (matching to something NOT on the list) "
            "are forbidden. Missed matches are recoverable; the user can map "
            "manually.\n"
