...
- **Event-loop offload on cart push** (session 83): `/order/submit` is an `async def` route, but `add_to_cart` is `requests.put(..., timeout=15)` — a blocking sync HTTP call. With a single uvicorn async worker, this freezes the entire app while waiting on Kroger (user-visible as "whole app stalls on Confirm Order, recovers a few minutes later"). Fix: wrap the call in `anyio.to_thread.run_sync` inside a `release_db_during_io()` block so the worker yields to other requests and the DB pool slot is freed during the wait. Reacquire `conn` via `_conn()` in the exception branch — the context manager closed the original.
- **Stamp-vs-SELECT chokepoint parity** (session 83): the SELECT that builds the Kroger payload includes `AND COALESCE(receipt_status, '') = '' AND checked = 0 AND have_it = 0 AND removed = 0`, but the pre-Kroger `UPDATE grocery_items SET submitted_at = CURRENT_TIMESTAMP` originally filtered on only `product_upc != '' AND ordered = 1 AND submitted_at IS NULL`. A row carrying a stale `receipt_status='not_fulfilled'` from a prior trip — common after the user re-selects a product on a previously demoted row — was excluded from the Kroger payload but still stamped as submitted. User-visible: the item disappears from the order page as if shipped, but the Kroger cart never received it. Fix: stamp UPDATE mirrors the SELECT's full filter, and (upstream) `/order/select`'s in-place UPDATE now clears `receipt_status`/`receipt_acknowledged`/`receipt_item`/`receipt_upc`/`receipt_price` when re-selecting a product so a re-picked row enters the next trip clean.
