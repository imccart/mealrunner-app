...
- **Followup deferred**: add `payment_intent_data={'statement_descriptor_suffix': 'TIP JAR'}` to one-time tip Checkout Sessions in `stripe_client.py` so customers see "ALETHEIA APPS TIP JAR" on their statement instead of just "ALETHEIA APPS" (chargeback-risk reduction). Subscriptions inherit account-level descriptor — different mechanism if needed.
